...
- `agent-service` 已落地 Plan-Execute、ReAct 工具循环、ReflectionCritic、`agentTrace`、SSE 流式输出、会话历史、会话记忆、MCP 工具接入、A2A 形态的多 Agent 入口和反馈记录 API。
...
`/api/chat/multi-agent`、`/api/chat/multi-agent/stream` 与 A2A 相关接口是并行探索入口；默认生产问答链路仍以 `/api/chat` 为准。
...
默认 `rag.memory.provider` 为 `in-memory`，可切换为 `redis` 或 `postgres`；`rag.memory.summary-mode` 默认为 `window`，也可以设置为 `llm`。
...
- 新增 SSE 流式聊天、多 Agent 流式入口、A2A 形态接口、会话历史、会话记忆、MCP 管理、Trace 查询和反馈记录。
