...
public class LlmChatClient implements LlmGateway {
    ...
    public boolean isConfigured() {
        AuthoringProperties.Llm llm = configuration();
        return llm != null
                && llm.apiKey() != null
                && !llm.apiKey().isBlank()
    ...
    public String complete(String systemPrompt, String userPrompt, double temperature, int maxTokens) {
        AuthoringProperties.Llm llm = configuration();
        if (!configured(llm)) {
            throw new IllegalStateException("authoring.llm.api-key is required for answer generation.");
        }
    ...
    private String completeOpenAiCompatible(AuthoringProperties.Llm llm, String systemPrompt, String userPrompt, double temperature, int maxTokens) {
    ...
    private String completeAnthropicCompatible(AuthoringProperties.Llm llm, String systemPrompt, String userPrompt, int maxTokens) {
    ...
    private AuthoringProperties.Llm configuration() {
        return runtimeModelConfigurationService.activeConfiguration();
    }

    private boolean configured(AuthoringProperties.Llm llm) {
        return llm != null
                && llm.apiKey() != null
                && !llm.apiKey().isBlank()
                && ((llm.openaiCompatible() != null && !llm.openaiCompatible().baseUrl().isBlank())
    ...
    private RestClient openAiClient(AuthoringProperties.Llm llm) {
        return restClientBuilder.clone().baseUrl(llm.openaiCompatible().baseUrl())
                .requestInterceptor(tracePropagationInterceptor).build();
    }

    private RestClient anthropicClient(AuthoringProperties.Llm llm) {
