...
public class RuntimeModelConfigurationService {
    ...
    public record ModelSnapshot(String provider, String model, String encryptedApiKey, double temperature,
                                int maxTokens, String openAiBaseUrl, String anthropicBaseUrl) { }

    private final JdbcTemplate jdbcTemplate;
    private final ConfigSecretCipher cipher;
    private final AuthoringProperties properties;
    private volatile AuthoringProperties.Llm active;
    private final ThreadLocal<AuthoringProperties.Llm> executionConfiguration = new ThreadLocal<>();

    public RuntimeModelConfigurationService(JdbcTemplate jdbcTemplate, ConfigSecretCipher cipher, AuthoringProperties properties) {
    ...
    public AuthoringProperties.Llm activeConfiguration() {
        AuthoringProperties.Llm scoped = executionConfiguration.get();
        return scoped != null ? scoped : active == null ? properties.llm() : active;
    }

    public ModelSnapshot snapshot() {
        AuthoringProperties.Llm value = activeConfiguration();
        return new ModelSnapshot(value.provider(), value.model(), cipher.encrypt(value.apiKey()), value.temperature(),
                value.maxTokens(), value.openaiCompatible().baseUrl(), value.anthropicCompatible().baseUrl());
    }

    public <T> T withSnapshot(ModelSnapshot snapshot, Supplier<T> action) {
        executionConfiguration.set(new AuthoringProperties.Llm(snapshot.provider(), snapshot.model(),
                cipher.decrypt(snapshot.encryptedApiKey()), snapshot.temperature(), snapshot.maxTokens(),
                new AuthoringProperties.CompatibleEndpoint(snapshot.openAiBaseUrl()),
                new AuthoringProperties.CompatibleEndpoint(snapshot.anthropicBaseUrl())));
        try {
            return action.get();
        } finally {
            executionConfiguration.remove();
        }
    }
    ...
    private void refreshActive() {
        ...
        active = "ANTHROPIC_COMPATIBLE".equals(row.protocol())
                ? new AuthoringProperties.Llm("runtime", row.model(), apiKey, row.temperature(), row.maxTokens(), new AuthoringProperties.CompatibleEndpoint(""), new AuthoringProperties.CompatibleEndpoint(row.baseUrl()))
                : new AuthoringProperties.Llm("runtime", row.model(), apiKey, row.temperature(), row.maxTokens(), new AuthoringProperties.CompatibleEndpoint(row.baseUrl()), new AuthoringProperties.CompatibleEndpoint(""));
    }
