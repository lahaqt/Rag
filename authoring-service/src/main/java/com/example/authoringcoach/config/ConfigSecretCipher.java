...
public class ConfigSecretCipher {
    ...
    public ConfigSecretCipher(@Value("${AUTHORING_CONFIG_ENCRYPTION_KEY:}") String masterKey) {
        this.masterKey = masterKey == null ? "" : masterKey;
    }

    public String encrypt(String value) {
        if (value == null || value.isBlank()) return "";
        if (masterKey.isBlank()) throw new IllegalStateException("AUTHORING_CONFIG_ENCRYPTION_KEY is required to save runtime secrets");
        try {
    ...
    public String decrypt(String value) {
        if (value == null || value.isBlank()) return "";
        if (masterKey.isBlank()) throw new IllegalStateException("AUTHORING_CONFIG_ENCRYPTION_KEY is required to read runtime secrets");
        try {
