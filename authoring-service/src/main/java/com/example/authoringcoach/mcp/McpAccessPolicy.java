...
public class McpAccessPolicy {
    ...
    public McpAccessPolicy(
            @Value("${authoring.mcp.runtime-registration-enabled:false}") boolean runtimeRegistrationEnabled,
            @Value("${authoring.mcp.direct-tool-call-enabled:false}") boolean directToolCallEnabled,
            @Value("${authoring.mcp.runtime-allowed-http-hosts:}") String allowedRuntimeHttpHosts
    ) {
    ...
    public void requireDirectToolCallsAllowed() {
        if (!directToolCallEnabled) {
            throw forbidden("Direct MCP tool calls are disabled; use a course-bound coaching review");
        }
    }
