# RAG 存储层 storage-layer

`storage-layer` 是知识库系统的存储层模块，负责知识库元数据、原始文档、文档解析、文本分块、异步索引任务、向量入库和向量检索。

本模块不负责大模型问答编排，也不直接生成最终回答。问答/Agent 模块应通过存储层提供的检索接口获取相关 chunk，再组织 Prompt 调用大模型。
...
```txt
docs/storage-layer-architecture.md
docs/chunking-strategies.md
```
