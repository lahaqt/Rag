...
type Review = { id: string; revisionId: string; status: 'COMPLETED' | 'INSUFFICIENT_EVIDENCE' | 'FAILED'; overallScore: number | null; dimensions: Dimension[]; evidence: Evidence[]; summary: string; failureReason: string; createdAt: string }
...
function App() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setReviews(latest ? await api<Review[]>(`/api/authoring/revisions/${latest.id}/reviews`) : [])
  ...
  const downloadReport = async () => {
    if (!activeProject) return
    try {
      const overview = await api<{ project: Project; course: Course; artifacts: Array<{ artifact: Artifact; revisionCount: number; latestReview?: Review }> }>(`/api/authoring/projects/${activeProject.id}/overview`)
      const markdown = [`# ${overview.project.title}`, `Course: ${overview.course.name}`, '', ...overview.artifacts.map(item => `## ${item.artifact.title}\nRevisions: ${item.revisionCount}\n${item.latestReview?.summary ?? 'No coaching review yet.'}`)].join('\n')
  ...
  const renderStudent = () => {
    ...
    if (studentPage === 'report') return <ReportPage project={activeProject} artifacts={artifacts} reviews={reviews} onDownload={downloadReport} />
...
function HistoryPage({ revisions, reviews }: { revisions: Revision[]; reviews: Review[] }) { return <section className="page-section"><p className="section-kicker">Revision history</p><h2>Evidence of your development</h2>{revisions.length === 0 ? <p className="empty-inline">Request coaching to freeze the first revision.</p> : <ol className="timeline">{revisions.map(revision => <li key={revision.id}><div><strong>Revision {revision.revisionNumber}</strong><span>{new Date(revision.createdAt).toLocaleString()}</span></div><p>{revision.title}</p>{reviews.find(review => review.revisionId === revision.id) && <span className="status-badge">Review completed</span>}</li>)}</ol>}</section> }
function ReportPage({ project, artifacts, reviews, onDownload }: { project: Project | null; artifacts: Artifact[]; reviews: Review[]; onDownload: () => void }) { return <section className="page-section"><p className="section-kicker">Project report</p><div className="report-heading"><div><h2>{project?.title ?? 'Select a project'}</h2><p className="muted">A consolidated view of revisions, evidence, and coaching outcomes.</p></div><button className="primary-button" disabled={!project} onClick={onDownload}>Download Markdown report</button></div><div className="metric-grid"><article><span>Artifacts</span><strong>{artifacts.length}</strong></article><article><span>Completed reviews</span><strong>{reviews.filter(item => item.status === 'COMPLETED').length}</strong></article><article><span>Latest formative score</span><strong>{reviews.at(-1)?.overallScore ?? '—'}</strong></article></div><p className="muted">Scores are formative signals, not course grades. Use the revision timeline and cited evidence to demonstrate how your work evolved.</p></section> }
