...
@Component
@ConditionalOnProperty(prefix = "content.lexical", name = "provider", havingValue = "elasticsearch", matchIfMissing = true)
public class ElasticsearchLexicalSearchStore implements LexicalSearchStore {
    ...
    public ElasticsearchLexicalSearchStore(ElasticsearchClient client, ContentProperties properties) {
        this.client = client;
        this.indexName = validateIndexName(properties.lexical().indexName());
    }

    @Override
    public void upsertDocument(CourseMaterial document) {
        ensureIndex();
        deleteMaterial(document.getCourseContentSpaceId(), document.getId());
        ...
        try {
            BulkResponse response = client.bulk(builder -> {
                ...
                for (DocumentChunk chunk : document.getChunks()) {
                    builder.operations(operation -> operation.index(index -> index
                            .id(materialId(chunk.getCourseContentSpaceId(), chunk.getMaterialId(), chunk.getId()))
            ...
            if (response.errors()) {
                for (BulkResponseItem item : response.items()) {
                    if (item.error() != null) {
                        log.warn("Elasticsearch lexical index item failed. index={} materialId={} reason={}",
    ...
    @Override
    public List<VectorSearchMatch> search(String courseId, String query, int topK) {
        ensureIndex();
        try {
                ...
                if (!exists) {
                    client.indices().create(builder -> builder
                            ...
                            .mappings(mappings -> mappings
                                    .properties("courseId", property -> property.keyword(keyword -> keyword))
                                    .properties("materialId", property -> property.keyword(keyword -> keyword))
    ...
    private String materialId(String courseId, String materialId, String chunkId) {
