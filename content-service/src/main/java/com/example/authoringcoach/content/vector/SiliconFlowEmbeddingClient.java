...
import com.example.authoringcoach.content.observability.TracePropagationInterceptor;
...
@Component
@ConditionalOnProperty(prefix = "content.vector.embedding", name = "provider", havingValue = "siliconflow", matchIfMissing = true)
public class SiliconFlowEmbeddingClient implements EmbeddingClient {
    ...
    public SiliconFlowEmbeddingClient(
        ...
        ContentProperties.Embedding embedding = properties.vector().embedding();
        if (embedding.apiKey().isBlank()) {
            throw new IllegalStateException("content.vector.embedding.api-key is required when provider is siliconflow.");
        }
