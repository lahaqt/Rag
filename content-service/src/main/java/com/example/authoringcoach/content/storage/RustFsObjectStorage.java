...
@Component
@ConditionalOnProperty(prefix = "content.object-storage", name = "provider", havingValue = "rustfs", matchIfMissing = true)
public class RustFsObjectStorage implements ObjectStoragePort {
    private final MinioClient client;
    private final String bucket;

    public RustFsObjectStorage(ContentProperties properties) {
        ContentProperties.ObjectStorage objectStorage = properties.objectStorage();
        if (objectStorage.accessKey().isBlank() || objectStorage.secretKey().isBlank()) {
            throw new IllegalStateException("CONTENT_RUSTFS_ACCESS_KEY and CONTENT_RUSTFS_SECRET_KEY are required when content.object-storage.provider=rustfs");
        }
