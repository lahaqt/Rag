...
@Component
@ConditionalOnProperty(prefix = "content.lexical", name = "provider", havingValue = "memory")
public class Bm25ChunkSearcher implements LexicalSearchStore {
    ...
    public Bm25ChunkSearcher(CourseContentMetadataStore metadataStore) {
        this.metadataStore = metadataStore;
    }

    @Override
    public void upsertDocument(com.example.authoringcoach.content.model.CourseMaterial document) {
    ...
    public void deleteMaterial(String courseId, String materialId) {
        // Metadata-backed implementation observes deletes through CourseContentMetadataStore.
    }
    ...
    public List<VectorSearchMatch> search(String courseId, String query, int topK) {
        ...
        List<ScoredChunk> chunks = metadataStore.listMaterials(courseId).stream()
    ...
    private VectorSearchMatch toMatch(ScoredChunk chunk, double score) {
        ...
        return new VectorSearchMatch(
                documentChunk.getCourseContentSpaceId(),
                documentChunk.getMaterialId(),
                documentChunk.getId(),
                documentChunk.getChunkIndex(),
                documentChunk.getMaterialName(),
